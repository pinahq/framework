...
Pina Framework - PHP фреймворк для разработки RESTfull, stateless приложений, с встроенной поддержкой очередей (gearman) и асинхронной обработкой событий. На уровне работы с БД Pina поддерживает автоматическую генерацию структуры таблиц и триггеров на основании классов модели (как альтернативу миграциям).
...
Генерировать структуру БД и триггеры на основе класса модели.
Единообразный универсальный роутинг, основанный на принципе “коллекция/ресурс/коллекция…”, позволяющий отказаться от больших таблиц роутинга и оперировать только RESTfull контроллерами.
...
Для начала работы нужно скачать bootstrap версию приложения (скоро будет на гитхабе и в компостере), обновить зависимости через composer, прописать корректные настройки адреса сайта и БД, и запустить php pinacli.php system.update для обновления БД.
